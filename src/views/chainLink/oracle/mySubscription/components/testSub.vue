<template>
    <a-modal v-model:visible="props.showTestSub" title="Test" :footer="null">
        ...
        <a-form class="modal-form" :model="formData" ref="formRef" :rules="formRules" layout="vertical">
            <a-form-item label="Subscription" name="subscription">
                <a-select dropdownClassName="modal-select-dropdown" @change="setSubscription"
                    v-model:value="formData.subscription" placeholder="Please select a subscription" autocomplete="off"
                    :options="subOptions" allow-clear></a-select>
            </a-form-item>
            <a-form-item label="Consumers" name="consumers">
                <a-select dropdownClassName="modal-select-dropdown" @change="setConsumers" show-search
                    v-model:value="formData.consumers" placeholder="Please input contract address" autocomplete="off"
                    :options="conOptions" allow-clear></a-select>
                <div v-if="false" class="w-[100%] bg-[#FFF9F2] p-[10px] mt-[10px]"
                    style="border:1px solid #E2B578;border-radius: 8px;">
                    The contract address needs to be added to the current Subsripion to complete the test, please ask
                    whether to continue to complete the binding relationship.
                    <div class="flex justify-end cursor-pointer open-link-css" @click="bind">Binding</div>
                </div>
            </a-form-item>
            <a-form-item label="Secrets Loaction" name="secretsLoaction">
                <a-select dropdownClassName="modal-select-dropdown" @change="setLoaction" v-model:value="formData.loaction"
                    placeholder="Please select a loaction" autocomplete="off"
                ...
                <div>
                    <a-input-group>
                        <a-row :gutter="8" v-for="item in formData.secretArr" class="mt-[5px]">
                            <a-col :span="11">
                                <a-input class="modal-input" v-model:value="item.secretName" allow-clear
                                    autocomplete="off" />
                            </a-col>
                            <a-col :span="11">
                                <a-input class="modal-input" v-model:value="item.secretValue" allow-clear
                                    autocomplete="off" />
                            </a-col>
    ...
    <a-modal v-model:visible="confirmShow" title="Confirm again" :footer="null">
        ...
        <div class="text-center flex justify-center mt-[20px]">
            <a-button class="done-btn" style="margin-right: 20px;background: transparent;color:#E2B578"
                @click="cancelToCheck">Recheck</a-button>
