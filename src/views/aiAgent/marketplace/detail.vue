<template>
  <div :class="theme.themeValue === 'dark' ? 'dark-css' : ''">
    ...
    <div class="bg-[#F5F6F8] dark:bg-[#0E0E0E] rounded-[12px] pb-[20px]">
      <div class="bg-[#EBEBEC] dark:bg-[#161616] rounded-tl-[12px] rounded-tr-[12px] p-[30px] flex justify-between items-center">
        <div class="flex justify-center">
          <img :src="getImageURL(`${detailInfo.logo}`)" class="h-[100px] w-[100px] rounded-full mr-[20px]"/>
          <div class="font-semibold text-[#656D7A]">
            <div><label class="text-[21px] text-[#000000] dark:text-[#FFFFFF] mr-[10px]">{{ detailInfo.title }}</label><label class="text-[16px]">{{ detailInfo.name }}</label></div>
      ...
      <div class="bg-[#EBEBEC] dark:bg-[#161616] rounded-[12px] m-[20px] mb-0 p-[20px] max-h-[635px] overflow-y-auto text-[14px] font-light" v-html="detailInfo.content.replace(/\n/g, '<br>')"></div>
    </div>
  </div>
