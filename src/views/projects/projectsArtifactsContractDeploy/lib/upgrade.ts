...
export async function getITransparentUpgradeableProxyFactory(
    ...
    return new ethers.ContractFactory(ITransparentUpgradeableProxy.abi, ITransparentUpgradeableProxy.bytecode, signer);
}
...
export async function upgradeProxyContract(provider: ethers.providers.Web3Provider, abi: ContractInterface, bytecode: BytesLike | { object: string }, initializeMethod: string, args: string[], proxyAddress: string) {
    const signer = provider.getSigner()
    let factory = new ethers.ContractFactory(abi,bytecode,signer)
    const contract = await factory.deploy()
    await contract.deployTransaction.wait();
    ...
    if (isEmptySlot(adminAddress) || adminBytecode === '0x') {
        ...
        if(initializeMethod === ''){
            ...
            const fragment = factory.interface.getFunction(initializeMethod)
            const call =  factory.interface.encodeFunctionData(fragment,args)
            const tx = await proxy.upgradeToAndCall(contract.address,call)
            console.log(tx)
        }
