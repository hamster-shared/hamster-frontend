<template>
  <div v-if="resultInfo?.length || gasInfo || AiInfo"
    class="rounded-[12px] mt-[24px] dark:bg-[#1D1C1A] bg-[#ffffff] dark:text-white text-[#121211]">
    <div class="check-container">
      ...
      <div v-for="(item, index) in resultInfo" v-if="resultInfo?.length" :key="index"
        class="items-center grid grid-cols-4 pl-10 rounded-xl my-8 border border-[#B5B5B5] border-solid">
        <span class="text-base font-bold">{{ item?.title }}</span>
        <div v-for="content in item?.content" class="py-8">
          <div>
            <span class="text-4xl cursor-pointer" :id="content?.checkTool"
              @click="router.push(`${route.fullPath}/${content.checkTool}?reportId=${content.id}&currentName=${currentName.replace('#', '[')}`)">{{
                content.issues }}</span>
      ...
      <div v-if="gasInfo" class="grid grid-cols-4 p-10 rounded-xl my-8 border border-[#B5B5B5] border-solid">
        ...
        <div class="flex col-span-2">
          <div class="flex-1">
            ...
            <div>
              <span class="inline-block my-2 text-base font-medium">Contract: {{
                gasAnalysisReport?.message[0]?.contractName }}</span>
          ...
          <a-button id="view-detail" class="view-detail-btn" type="link"
            @click="router.push(`${route.fullPath}/gasInfoDetail?currentName=${currentName.replace('#', '[')}`)">View
            Detail</a-button>
        </div>
      </div>
